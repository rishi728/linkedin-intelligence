...
import { classifyAuto, isCampusOrg, rolesInText, titleKey } from "./intelligence";

type Case = [position: string, company: string, domain: string, fn: string, role?: string];

const CASES: Case[] = [
  ["Product Manager", "Google", "product", "product-management", "Product Manager"],
  ["Senior Product Manager", "Razorpay", "product", "product-management", "Senior Product Manager"],
  ...
  ["Operations Manager", "Delhivery", "operations", "business-ops", "Operations Manager"],
  ["Supply Chain Manager", "Unilever", "operations", "supply-chain", "Supply Chain Manager"],
  ...
  ["Backend Engineer", "Swiggy", "engineering", "software-engineering", "Backend Engineer"],
  ["SDE2", "Amazon", "engineering", "software-engineering", "Software Engineer"],
  ["Growth Manager", "CRED", "marketing", "growth", "Growth Manager"],
  ["Head of Developer Relations", "Postman", "technology", "developer-relations", "Developer Advocate"],
  ...
  ["Co-Founder & CEO", "Stealth Startup", "leadership", "founders", "Founder & CEO"],
  ...
  ["Management Trainee", "ITC Limited", "students", "early-career", "Management Trainee"],
  ["Software Engineer Intern", "Microsoft", "engineering", "software-engineering", "Software Engineering Intern"],
  ["Summer Intern", "Goldman Sachs", "students", "internships", "Intern"],
  ...
  ["Executive Member", "Business Club, NIT Warangal", "students", "campus", "Club / Team Member"],
  ["Joint Secretary", "Mechanical Engineering Association NITW", "students", "campus", "Club Secretary"],
  ["Joint Secretary", "Ministry of Finance, Government of India", "public", "government", "Civil Servant"],
  ["President", "180 Degrees Consulting BITS Goa", "students", "campus", "Club Secretary"],
  ...
  ["Assistant Manager", "HDFC Bank", "unclassified", "unspecified", "Assistant Manager"],
  ["PM", "Swiggy", "product", "product-management", "Product Manager"],
  ["Lead Product Manager", "Swiggy", "product", "product-management", "Lead Product Manager"],
  ["Key Account Manager", "Nestle", "sales", "sales", "Key Account Manager"],
  ["", "", "unclassified", "unclassified", "Role not shared"],
];

describe("classifyAuto: hierarchy", () => {
  it.each(CASES)("%s @ %s → %s / %s / %s", (position, company, domain, fn, role) => {
    const r = classifyAuto(position, company);
    const got = { domain: r.domain, fn: r.fn, ...(role ? { role: r.role } : {}) };
    expect(got, r.reasons.join(" · ")).toEqual({ domain, fn, ...(role ? { role } : {}) });
  });
});


describe("a title that is only a level says nothing about the work", () => {
  it("refuses to invent an area from a level-only title", () => {
    for (const [position, company] of [
      ["Assistant Manager", "Reliance"], ["Senior Manager", "Tata Motors"], ["Manager", "Nestle"],
      ["Director", "Siemens"], ["Vice President", "HDFC Bank"], ["Executive", "Godrej"],
    ]) {
      expect(classifyAuto(position, company), position).toMatchObject({ fn: "unspecified" });
    }
  });
  ...
  it("keeps the area when the employer only does one thing", () => {
    ...
    expect(classifyAuto("Senior Associate", "PwC")).toMatchObject({ domain: "strategy", fn: "management-consulting" });
  ...
  it("a level plus an area keeps the area", () => {
    expect(classifyAuto("Vice President Of Engineering", "Acme")).toMatchObject({ domain: "engineering" });
    expect(classifyAuto("Director of Engineering", "Acme")).toMatchObject({ domain: "engineering" });
    expect(classifyAuto("AVP - Data Science", "ICICI")).toMatchObject({ domain: "data-ai" });
    expect(classifyAuto("Senior Director, Product", "Adobe")).toMatchObject({ domain: "product" });
  });
...
describe("campus roles never read as corporate seniority", () => {
  it.each([
    ...
    expect(c).toMatchObject({ domain: "students", fn: "campus", campusOrg: true });
  ...
  it("leaves the same titles alone at a real employer", () => {
    expect(classifyAuto("Head of Marketing", "Acme Foods")).toMatchObject({ domain: "marketing" });
